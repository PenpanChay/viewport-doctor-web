...
const { STANDARD_BREAKPOINTS, BREAKPOINT_BANDS } = require('./viewportPresets');
 ...
 * regression needs.
 */
function markCandidatesInBrowser(maxCandidates) {
 ...
 */
function measureColumnsInBrowser() {
  function isVisible(el) {
    const style = getComputedStyle(el);
    if (style.display === 'none' || style.visibility === 'hidden' || Number(style.opacity) === 0) return false;
    const rect = el.getBoundingClientRect();
  ...
  document.querySelectorAll('[data-vp-track-id]').forEach((el) => {
    ...
    result[id] = columnsOf(el);
  });
  return result;
}

function nearestStandardBreakpoint(width) {
  let best = null;
 ...
 * @param {string} url
 * @param {{minWidth?: number, maxWidth?: number, height?: number, timeoutMs?: number, settleMs?: number, maxCandidates?: number}} [options]
 * @returns {Promise<{
 *   url: string,
 *   minWidth: number,
 *   maxWidth: number,
 ...
 * }>}
 */
async function discoverBreakpoints(browser, url, options = {}) {
  ...
  const base = { url, minWidth, maxWidth, bands: BREAKPOINT_BANDS, transitions: [] };
  ...
  try {
    ...
    const series = new Map(initial.map((c) => [c.id, [{ width: minWidth, columns: c.columns }]]));
...
module.exports = { discoverBreakpoints, nearestStandardBreakpoint, EXPECTED_TOLERANCE_PX };
