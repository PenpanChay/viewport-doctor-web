 ...
 * every helper it needs is declared inside it.
 *
 ...
 * lib/previewFix.js needs to answer "did applying this CSS actually help,
 * fully fix it, or do nothing" instead of just "is it still in the list".
 *
 * @returns {{issues: Array<{check: string, message: string, selector: string, rect: {x:number,y:number,width:number,height:number}, severity: number|null, details: {expected: string, actual: string, delta: string|null, extra?: Array<{label:string,value:string}>}|null}>, scrollX: number, scrollY: number}}
 */
function runChecksInBrowser() {
  ...
  // between them).
  function addIssue(check, message, el, severity, details) {
    issues.push({
      check,
      message,
  ...
  if (document.documentElement.scrollWidth > viewportWidth + TOLERANCE) {
    ...
    const byRightDesc = (a, b) => b.getBoundingClientRect().right - a.getBoundingClientRect().right;
  ...
  for (let i = 0; i < candidates.length; i++) {
    for (let j = i + 1; j < candidates.length; j++) {
      ...
      if (smallerArea > 0 && overlapArea / smallerArea > 0.3) {
        ...
        // lib/suggestFixes.js's extractGridCollision(), which parses this
  ...
  if (fixedOrSticky.length > 0) {
    ...
    fixedOrSticky.forEach((bar) => {
      ...
      // "add this much padding" note (see suggestFixes.js) - a bar floating
  ...
  function parseColor(str) {
    ...
    if (!parseColor._ctx) {
      ...
      canvas.width = 1;
      canvas.height = 1;
      parseColor._ctx = canvas.getContext('2d', { willReadFrequently: true });
    }
...
module.exports = { runChecksInBrowser, CHECK_TYPES };
