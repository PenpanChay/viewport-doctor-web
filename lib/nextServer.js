 ...
 * doesn't build.
 *
 * @param {string} rootDir project root (containing .next/)
 * @param {number} [port] fixed port, or 0 to let the OS pick a free one
 * @returns {Promise<{port: number, close: () => Promise<void>}>}
 */
function startNextServer(rootDir, port = 0) {
  return new Promise((resolve, reject) => {
    ...
    function settleResolve(actualPort) {
      if (settled) return;
      settled = true;
