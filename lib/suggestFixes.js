 ...
 * type in lib/checks.js, plus a scoping note computed by cross-referencing
...
function overlappingElementsBaseFix(message) {
...
function escapeCssIdentifier(name) {
  return name.replace(/[^a-zA-Z0-9_-]/g, (ch) => `\\${ch}`);
}

// `describe(el)` in lib/checks.js builds selectors like
...
function extractAncestorSelector(message) {
...
function extractOtherSelector(message) {
...
// (grid-column-start/grid-row-start: 1/1)." - added by lib/checks.js only
...
function extractGridCollision(message) {
...
function extractOffscreenEdge(message) {
...
function extractFixedBarInfo(message) {
...
function extractModalWidth(message) {
...
function extractTapTargetMin(message) {
...
function extractCrampedAxis(message) {
...
function extractNaturalImageSize(message) {
...
function buildFixCode(check, { selector, message, realSelectors }) {
  ...
  switch (check) {
    ...
    case 'overlapping-elements': {
      ...
      return [
        ...
        .filter((line) => line !== null) // keep the '' blank-line separator; only drop the missing `other` comment
        .join('\n');
    }
    case 'text-overflow':
      return [
...
function addPasteLocationNote(code, check) {
...
function wrapCodeForScope(code, { scoped, breakpointMaxWidth }) {
...
// whose own parent doesn't overflow" (see lib/checks.js) - by design, that
...
// was named each time.
const PAGE_LEVEL_CHECKS = new Set(['horizontal-overflow']);

function formatViewportList(list) {
  return list.map((v) => `${v.label} (${v.width}px)`).join(', ');
}

function scopingNote({ scoped, breakpointHint, brokenViewports, okViewports }) {
  if (!scoped) {
...
/**
 * @param {{viewports: Array<{label:string, width:number, height:number, issues: Array<{check:string, message:string, selector:string}>, navigationError?: string}>}} pageResult
 *   The per-page result shape scanAllViewports() produces (before or after
 *   it's wrapped with a `url`) - only `viewports` is read.
 * @returns {Array<{
 *   check: string,
 *   selector: string|null,
 *   message: string,
 *   elementVaries: boolean,
 *   brokenViewports: Array<{label:string,width:number,height:number}>,
 *   okViewports: Array<{label:string,width:number,height:number}>,
 *   scoped: boolean,
 ...
 * }>}
 */
function buildFixSuggestions(pageResult) {
  if (!pageResult || !Array.isArray(pageResult.viewports)) return [];

  const scannedViewports = pageResult.viewports
    ...
    for (const issue of vp.issues) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          ...
          // Page-level checks throw away the single `selector` field above
          // (there isn't one bug-wide selector to report), but the code
          // snippet still needs to target *something* real - so keep every
          // distinct selector actually observed, and which viewport(s) each
          // one showed up at, instead of falling back to a fictional class
          // name that matches nothing in the real page.
      ...
      groups.get(key).brokenViewportsByLabel.set(vp.label, { label: vp.label, width: vp.width, height: vp.height });
      if (pageLevel && issue.selector) {
        const bucket = groups.get(key).observedSelectorsByLabel;
  ...
  for (const group of groups.values()) {
    ...
    const realSelectors = group.observedSelectorsByLabel
...
module.exports = { buildFixSuggestions, PAGE_LEVEL_CHECKS };
