 ...
 */
function drawOverlayInBrowser({ issues, scrollX, scrollY }: OverlayPayload) {
  ...
  const badgeSlotsUsed = new Map<string, number>();
 ...
 * fifteen responsive-layout/legibility checks, and captures a full-page
 * screenshot with a highlight box around every flagged element.
 */
export async function scanViewport(
  browser: Browser,
  url: string,
  viewport: ViewportSize,
  options: ScanViewportOptions = {}
): Promise<ScanViewportResult> {
...
export async function scanAllViewports(options: ScanAllViewportsOptions): Promise<ScanAllViewportsResult> {
  ...
  try {
    for (const url of urls) {
      const results: ScanResultPerViewport[] = [];
      ...
      // global fix - see lib/suggestFixes.ts.
