...
import { previewFix, evaluatePreview } from '../lib/previewFix';
import { buildFixSuggestions } from '../lib/suggestFixes';
import { startNextServer, type NextServerHandle } from '../lib/nextServer';
import { scanViewport } from '../lib/scanViewport';
import type { ComparableIssue } from '../lib/types';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PROJECT_ROOT = path.join(__dirname, '..');
const MOBILE = { width: 390, height: 844 };

function issue(check: string, selector: string | null, severity: number): ComparableIssue {
...
describe('previewFix (real browser, real Next.js server, no mocking)', () => {
  ...
  it.each(['text-overflow', 'overflowing-image', 'overlapping-elements'])(
    ...
    async (check) => {
      ...
      // work, lib/checks.ts and lib/suggestFixes.ts were extended to
      ...
      // test/scanViewport.test.ts) - `find()` below picks up the first one
  ...
  it('rejects an unpreviewable check before ever opening a browser (broken-image has no CSS fix)', async () => {
    ...
    const { NOT_PREVIEWABLE_CHECKS } = await import('../lib/previewFix');
