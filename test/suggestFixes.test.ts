...
import { buildFixSuggestions } from '../lib/suggestFixes';
import type { FixSuggestionIssue, FixSuggestionsViewport, Rect } from '../lib/types';

const MOBILE: Pick<FixSuggestionsViewport, 'label' | 'width' | 'height'> = { label: 'Mobile', width: 390, height: 844 };
const DESKTOP: Pick<FixSuggestionsViewport, 'label' | 'width' | 'height'> = { label: 'Desktop', width: 1440, height: 900 };

function issueFixture(check: string, message = `${check} message`, selector = `div.${check}`, rect?: Rect): FixSuggestionIssue {
...
describe('buildFixSuggestions (pure, no browser/server needed)', () => {
  ...
  it('handles a missing/malformed pageResult gracefully', () => {
    // @ts-expect-error - deliberately exercising the defensive guard against malformed input
    expect(buildFixSuggestions(undefined)).toEqual([]);
    // @ts-expect-error - same: `{}` has no `viewports` at all
  ...
  it('escapes Tailwind arbitrary-value class names (brackets, slashes) so the generated fixCode selector is valid CSS', () => {
    ...
    // checks.ts produces for a Tailwind arbitrary-value class - but `[`,
  ...
  it('suggests grid-column-start (not margin) for an overlapping-elements bug that names a shared grid cell', () => {
    // lib/checks.ts only appends this sentence when both flagged elements
